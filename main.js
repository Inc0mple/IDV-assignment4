// Set up dimensions - increased height and adjusted margins
const width = 1000;
const height = 700; // Increased from 600
const margin = { top: 40, right: 20, bottom: 30, left: 20 }; // Increased top margin
...
Promise.all([
  ...
  svg.append("g")
    ...
    .on("click", function(event, d) {
        ...
        // Modify your zoomed function to use the saved reference
        svg.transition()
          .duration(750)
          .on("end", function() {
            ...
            // Highlight this zone with stored reference
            d3.select(currentElement).style("stroke-width", 1.5);
          });
      });
...
function createLegend(colorScale, maxValue) {
  ...
  const legendAxis = d3.axisBottom(legendScale)
    .ticks(6)
    .tickFormat(d => d === 0 ? "0" : d >= 1000 ? `${d/1000}k` : d);
